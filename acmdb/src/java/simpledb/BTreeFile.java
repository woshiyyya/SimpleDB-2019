...
public class BTreeFile implements DbFile {
    ...
    /**
     * RootPtrPage -> RootPage <-> [internal pages] <-> [Leaf pages <-> Leaf pages]
     */
//    private BTreeInternalPage getParentWithEmptySlots(TransactionId tid, BTreePageId parentId, HashMap<PageId, Page> dirtypages, Field f)
//            throws DbException, IOException, TransactionAbortedException {
//        if (parentId.pgcateg() == BTreePageId.ROOT_PTR) {
//            BTreePageId rootPrrPageId = BTreeRootPtrPage.getId(tableid);
//            BTreeRootPtrPage rootPtrPage = (BTreeRootPtrPage) getPage(tid, dirtypages, rootPrrPageId, Permissions.READ_WRITE);
//
//            // create a new page as the root, since current root is full
//            BTreeInternalPage newRootPage = (BTreeInternalPage) getEmptyPage(tid, dirtypages, BTreePageId.INTERNAL);
//            rootPtrPage.setRootId(newRootPage.pid);
//            return newRootPage;
//        } else {
...
//                parentPage = splitInternalPage(tid, dirtypages, parentPage, f);
//            return parentPage;
//        }
//    }
    ...
    protected BTreeLeafPage splitLeafPage(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreeLeafPage page, Field field)
        ...
        // prepare a new leaf page
        BTreeLeafPage newLeafPage = (BTreeLeafPage) getEmptyPage(tid, dirtypages, BTreePageId.LEAF);
        // find the middle key which have to be "copied up"
        int num_tuples = page.getMaxTuples();
        Field MiddleKey = page.getTuple(num_tuples / 2).getField(keyField);
        // find a valid parent page to insert middle key
        BTreeInternalPage ParentPage = getParentWithEmptySlots(tid, dirtypages, page.getParentId(), MiddleKey);
        // insert the middle key TODO: 如果middle key正好处在连续相同元素处有问题 [5566677]->[556]+[6677]
        ParentPage.insertEntry(new BTreeEntry(MiddleKey, page.getId(), newLeafPage.getId()));
        // transport half Tuples onto new Leafpage
        ...
        // Update parent-child ptr & siblings ptr
        ...
        BTreePageId prev_right_pid = page.getRightSiblingId();
        page.setRightSiblingId(newLeafPage.getId());
        newLeafPage.setLeftSiblingId(page.getId());
        newLeafPage.setRightSiblingId(prev_right_pid);
        if (prev_right_pid != null) {
            BTreeLeafPage rightpage = (BTreeLeafPage) getPage(tid, dirtypages, prev_right_pid, Permissions.READ_WRITE);
    ...
    protected BTreeInternalPage splitInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages,
        ...
        // find the middle key which have to be "copied up"
        int num_entries = page.getMaxEntries();
        BTreeEntry MiddleEntry = null;

        // In order to reduce shifts caused by reverse insertion, we use left-right iterator.
        Iterator<BTreeEntry> iter = page.iterator();
        for (int i = 0; i < num_entries; i++) {
            BTreeEntry entry = iter.next();
            // Push up middle entry
            if (i == num_entries / 2) {
        ...
        ParentPage.insertEntry(new BTreeEntry(MiddleEntry.getKey(), page.getId(), newInternalPage.getId()));
    ...
    protected void stealFromLeafPage(BTreeLeafPage page, BTreeLeafPage sibling,
        ...
        assert sibling.getNumTuples() > page.getNumTuples();
        int steal_num = (sibling.getNumTuples() - page.getNumTuples()) / 2;
        int cnt = 0;
    ...
    protected void stealFromLeftInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages,
        ...
        // some code goes here
        assert page.getNumEntries() < leftSibling.getNumEntries();
        int steal_num = (leftSibling.getNumEntries() - page.getNumEntries()) / 2 - 1;

        BTreePageId rightmost_in_left = leftSibling.reverseIterator().next().getRightChild();
        BTreePageId leftmost_in_right = page.iterator().next().getLeftChild();
        BTreeEntry modifiedParentEntry = new BTreeEntry(parentEntry.getKey(), rightmost_in_left, leftmost_in_right);
        page.insertEntry(modifiedParentEntry);
        ...
        BTreeEntry pushUpEntry = iter.next();
    ...
    protected void stealFromRightInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages,
        ...
        assert page.getNumEntries() < rightSibling.getNumEntries();
        int steal_num = (rightSibling.getNumEntries() - page.getNumEntries()) / 2 - 1;

        BTreePageId rightmost_in_left = page.reverseIterator().next().getRightChild();
        BTreePageId leftmost_in_right = rightSibling.iterator().next().getLeftChild();
        BTreeEntry modifiedParentEntry = new BTreeEntry(parentEntry.getKey(), rightmost_in_left, leftmost_in_right);
        page.insertEntry(modifiedParentEntry);
    ...
    protected void mergeLeafPages(TransactionId tid, HashMap<PageId, Page> dirtypages,
        ...
        BTreePageId rightmost_pid = rightPage.getRightSiblingId();
        leftPage.setRightSiblingId(rightmost_pid);
        if (rightmost_pid != null) {
            BTreeLeafPage rightmost = (BTreeLeafPage) getPage(tid, dirtypages, rightmost_pid, Permissions.READ_WRITE);
            rightmost.setLeftSiblingId(leftPage.getId());
        }
    ...
    protected void mergeInternalPages(TransactionId tid, HashMap<PageId, Page> dirtypages,
        ...
        // Delete the entry in the parent corresponding to the two pages that are merging -
        // deleteParentEntry() will be useful here
        BTreePageId leftmost_in_right = rightPage.iterator().next().getLeftChild();
        BTreePageId rightmost_in_left = leftPage.reverseIterator().next().getRightChild();
        BTreeEntry pivotEntry = new BTreeEntry(parentEntry.getKey(), rightmost_in_left, leftmost_in_right);
        leftPage.insertEntry(pivotEntry);
